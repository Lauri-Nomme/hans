...
>
> PR 8's reference matrix exists to verify that each reference form survives
> REST → export archive → GH import **byte-for-byte** (and to observe which ones
> GEI/GitHub rewrite or auto-link). Labels `test-1`..`test5` are stable search
...
> refs). Adding PR 8 changes the corpus: the golden export (`ground-truth/`,
> 7 PRs) and the corpus REST dumps must be re-captured after rebuilding.
